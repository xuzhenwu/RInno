...
code_section <- function(iss, R_version = paste0(">=", R.version$major, ".", R.version$minor)) {
  ...
  R_versions <-
    c(
      unique(stats::na.omit(stringr::str_extract(readLines("https://cran.rstudio.com/bin/windows/base/", warn = F), "[1-9]\\.[0-9]+\\.[0-9]+"))),
      stats::na.omit(stringr::str_extract(readLines("https://cran.rstudio.com/bin/windows/base/old/", warn = F), "[1-9]\\.[0-9]+\\.[0-9]+")))
